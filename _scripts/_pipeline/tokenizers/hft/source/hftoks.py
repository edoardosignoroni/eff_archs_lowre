...
def filter_vocab(word_iter, vocab_name, freq_threshold=100):
    allwords = word_counts(word_iter)
    vocab = read_vocab(vocab_name)
    nextvocab, candid = vocab_from_segments(allwords, vocab)
    newvocab = Trie()
    for s,f,_ in vocab.items():
        gf = nextvocab.get_freq(s)
        if gf is not None and gf >= freq_threshold:
            newvocab.increment(s, gf)
        elif len(s) == 1:
            newvocab.increment(s, 1)
    return newvocab
...
def usage():
    ...
    print('       hftoks.py filter INVOCABFILE [FREQ_THRESHOLD] <INTEXT >OUTVOCABFILE')
    print('       hftoks.py help')
    sys.exit(1)
...
if __name__ == '__main__':
    if not sys.argv[1:]:
        usage()
    if sys.argv[1] not in ('learn', 'tokenize', 'filter', 'help'):
        usage()
    if sys.argv[1] == 'learn':
        if not sys.argv[3:]:
            usage()
        ...
        save_vocab(sys.argv[3], vocab)

    elif sys.argv[1] == 'help':
        print('cat src trg >jointext')
        print('hftoks.py learn jointext joinvocab')
        print('cat src |hftoks.py filter joinvocab MINFREQ >src.vocab')
        print('cat trg |hftoks.py filter joinvocab MINFREQ >trg.vocab')
        sys.exit(2)
        
    elif sys.argv[1] == 'filter':
        freq_thres = 100
        if sys.argv[3:]:
            freq_thres = int(sys.argv[3])
        words = open(0, encoding='utf8').read().split()
        newvocab = filter_vocab(words, sys.argv[2], freq_thres)
        save_vocab(1, newvocab)
